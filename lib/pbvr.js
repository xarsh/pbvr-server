...
export default {
  generateParticles(data, ensemble = 1) {
    const ensembles = [];
        ...
        for (let y = 0; y < data.y - 1; y++) {
          for (let x = 0; x < data.x - 1; x++) {
            ...
            for (let i = 0; i < rho; i++) {
              ...
              const scalar = cube.interpolateScalar(particlePosition) / data.max;
              const r = Math.sqrt(-Math.log(1 - scalar) / 0.5 / Math.PI / rho);

              particles.push([...position, scalar, r].map(round));
            }
          }
        }
      }
